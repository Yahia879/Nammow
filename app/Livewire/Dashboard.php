...
class Dashboard extends Component
{
    ...
    public function mount()
    {
        ...
        $activeTimeline = $user ? $user->timelines()->where('end_date', null)->first() : null;
    ...
    public function getEmployeeDiscounts()
    {
        ...
        $query = Discount::select('batch');

        if (config('database.default') === 'sqlite') {
            $this->latestBatch = $query->get()
                ->unique('batch')
                ->map(function ($item) {
                    $dates = explode(' to ', $item->batch);
                    $item->sort_date = $dates[0];
                    return $item;
                })
                ->sortByDesc('sort_date')
                ->first()?->batch;
        } else {
            $this->latestBatch = $query->orderByRaw("STR_TO_DATE(SUBSTRING_INDEX(batch, ' to ', 1), '%Y-%m-%d') DESC")
                ->limit(1)
                ->value('batch');
        }
        ...
        $this->batchDates = $this->latestBatch ? explode(' to ', $this->latestBatch) : [null, null];
