...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        if (!Schema::hasTable('company_manager_company')) {
        ...
        // 2. Transfer data (Only if the column still exists)
        if (Schema::hasColumn('company_managers', 'company_id')) {
            $managers = DB::table('company_managers')->get();
            foreach ($managers as $manager) {
                if ($manager->company_id) {
                    DB::table('company_manager_company')->insertOrIgnore([
